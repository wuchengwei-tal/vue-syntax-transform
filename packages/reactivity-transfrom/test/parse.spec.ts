    ...
    describe("source map", () => {
        test("style block", () => {
            ...
            consumer.eachMapping((mapping) => {
                expect(mapping.originalLine - mapping.generatedLine).toBe(
                    padding
                );
            });
        });
...
`;
    ...
    test("slotted detection", async () => {
        ...
        expect(
            parse(`<template>hi</template><style>h1{color:red;}</style>`)
                .descriptor.slotted
        ).toBe(false);
        expect(
            parse(
    ...
    describe("warnings", () => {
        ...
        test("should only allow single script element", () => {
            assertWarning(
                parse(
                    `<script>console.log(1)</script><script>console.log(1)</script>`
                ).errors,
